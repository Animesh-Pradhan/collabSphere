...
import { OrgMembersPayload } from "../types/types";
import { useEffect } from "react";

export const useMembersQuery = ({ all = false }: { all?: boolean }) => {
    const { page, limit, search, order, sortBy, fromDate, toDate, role, status, setMembers } = useMembersStore();

    const membersQuery = useAppQuery<OrgMembersPayload>(["org-members", page, limit, search, role, status, fromDate, toDate, sortBy, order, all], async () => {
        const res = await getOrgMembers({ page, limit, search, role, status, fromDate, toDate, sortBy, order, all });
        return res.data;
    }, {
