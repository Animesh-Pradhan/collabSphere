...
import { Controller, Get, Post, Body, Param, Delete, UseGuards, Req, UseInterceptors, UploadedFile, Query, BadRequestException, ForbiddenException, Patch } from '@nestjs/common';
...
import { AcceptInviteDto, BulkDeleteOrgMembersDto, GetOrgInvitationsQueryDto, InviteOrganisationMemberDto, UpdateOrgMemberDto } from './dto/other-helpert.dto';
...
export class OrganisationController {
  ...
  @ApiOperation({ summary: 'Create Organisation', description: 'Create a new organisation' })
  @Post()
  ...
  @Roles(Role.USER)
  @UseInterceptors(UploadInterceptor("ORGANISATION_LOGO"))
  @ApiOperation({ summary: 'Update Organisation', description: 'Update a existing organisation' })
  @Patch(':id')
  ...
  async resendInvite(@Req() req: Request, @Param('id') inviteId: string) {
    const orgId = req.user.ctx.orgId;
    const userId = req.user.sub;
    if (!orgId) throw new ForbiddenException('Organisation Scope Required');
    const data = await this.organisationService.resendInvitation(userId, orgId, inviteId);

    return { message: 'Invite resent successfully', data }
  }

  @Roles(Role.USER)
  @Patch('invite/:id/cancel')
  async cancelInvite(@Req() req: Request, @Param('id') inviteId: string) {
    const orgId = req.user.ctx.orgId;
    const userId = req.user.sub;
    if (!orgId) throw new ForbiddenException('Organisation Scope Required');

    const data = await this.organisationService.cancelInvite(userId, orgId, inviteId);
    return { message: 'Invite cancelled successfully', data }
  }
  ...
  @Roles(Role.USER)
  @ApiOperation({ summary: 'Update Organisation Member', description: 'Update a existing member of a organisation' })
  @Patch('member/:id')
  async updateOrgMember(@Req() req: Request, @Param('id') memberId: string, @Body() body: UpdateOrgMemberDto) {
    const orgId = req.user.ctx.orgId;
    const userId = req.user.sub;
    if (!orgId) throw new ForbiddenException('Organisation Scope Required to update a member');
    const data = await this.organisationService.updateOrgMember(orgId, userId, memberId, body);

    return { message: "Updated Succesfully", data }
  }


  @Roles(Role.USER)
  @ApiOperation({ summary: 'Bulk delete organisation members' })
  @Delete('members')
  async deleteMembers(@Req() req: Request, @Body() body: BulkDeleteOrgMembersDto) {
    const orgId = req.user.ctx.orgId;
    const userId = req.user.sub;
    if (!orgId) throw new ForbiddenException('Organisation Scope Required');
    const data = await this.organisationService.removeOrgMembers(orgId, userId, body.memberIds);

    return { message: `${data.result.count} Members removed successfully`, data: data.ids };
  }

  @Roles(Role.USER)
  @Roles(Role.SUPER_ADMIN)
  @ApiOperation({ summary: 'Delete Organisation', description: 'Delete a existing organisation' })
  @Delete(':id')
