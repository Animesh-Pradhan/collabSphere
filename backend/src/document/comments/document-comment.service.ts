...
export class DocumentCommentService {
    ...
    async updateComment(workspaceId: string, workspaceMemberId: string, documentId: string, commentId: string, updateData: UpdateDocumentCommentDto) {
        return this.prisma.$transaction(async (tx) => {
            ...
            if (comment.workspaceMemberId !== workspaceMemberId) {
                throw new ForbiddenException("You can only edit your own comment");
            }

            const updated = await tx.documentComment.update({
                where: { id: commentId },
                data: { comment: updateData.comment, editedAt: new Date() }
            });

            await this.documentActivity.log(tx, {
                workspaceId, documentId, workspaceMemberId, type: "COMMENT_CREATED", documentVersionId: version.id, commentId: updated.id,
