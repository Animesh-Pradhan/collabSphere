...
import { CreateDocumentCommentDto, UpdateDocumentCommentDto } from './dto/create-document-comment.dto';
...
export class DocumentCommentController {
    ...
    @ApiOperation({ summary: "Edit Comment", description: "Edit the text of your own comment." })
    @Patch(":commentId")
    async update(@Param("workspaceId") workspaceId: string, @Param("documentId") documentId: string, @Param("commentId") commentId: string, @Req() req: Request, @Body() updateCommentDto: UpdateDocumentCommentDto) {
        const workspaceMemberId = req.workspaceMember?.id as string;
        const data = await this.documentCommentService.updateComment(workspaceId, workspaceMemberId, documentId, commentId, updateCommentDto);
