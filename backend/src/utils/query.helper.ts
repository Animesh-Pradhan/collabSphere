...
export async function executePaginatedQuery({ model, prismaQuery, page, limit, all }: {
    model: any;
    prismaQuery: any;
    page: number;
    limit: number;
    ...
    if (all) {
        const data = await model.findMany(prismaQuery);

        return {
            data,
            meta: { totalItems: data.length, page: 1, limit: data.length, totalPages: 1 }
        };
    }
