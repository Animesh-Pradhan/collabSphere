...
import { Controller, Get, Post, Body, Param, Delete, UseGuards, Req, ForbiddenException, Query, Patch, BadRequestException } from '@nestjs/common';
...
export class WorkspaceController {
  ...
  @Get('all')
  async getOrganisationWorkspaces(@Req() req: Request, @Query() query: GetMyWorkspacesQueryDto) {
    const userId = req.user.sub;
    const organisationId = req.user.ctx.orgId;

    if (!organisationId) throw new BadRequestException('Organisation context required');

    const data = await this.workspaceService.getOrganisationWorkspaces(userId, organisationId, query);

    return {
      message: "Organisation workspaces fetched successfully",
      data: { workspaces: data.data, meta: data.meta }
    };
  }

  @ApiOperation({ summary: "Get All Workspaces", description: "Get all workspaces by organisation or personal" })
  @Get()
  async getMyWorkspaces(@Req() req: Request, @Query() query: GetMyWorkspacesQueryDto) {
    const userId = req.user?.sub;
    const organisationId = req.user?.ctx.mode === "ORG" ? req.user?.ctx.orgId : null;

    const data = await this.workspaceService.getMyWorkspaces(userId, organisationId, query);
    return {
      message: "All workspaces retrieved successfully.", data: {
  ...
  @Get(":id/members")
  async getWorkspaceMembers(@Param("id") workspaceId: string, @Req() req: Request, @Query() query: GetWorkspaceMembersQueryDto) {
    const userId = req.user.sub;
    const role = req.user.role;

    const data = await this.workspaceService.getWorkspaceMembers(workspaceId, userId, role, query);
    return { message: 'Workspace Members fetched successfully', data: { workspacesMembers: data.data, meta: data.meta } }
  }
